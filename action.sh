...
cd Ultroid
chmod +x startup
apt-get update -y && apt install ffmpeg mediainfo neofetch -y && python3 -m pip install --upgrade pip setuptools wheel && python3 -m pip install --no-cache-dir --ignore-installed --force-reinstall --no-warn-conflicts -U -r req* && python3 -m pip install --no-cache-dir --ignore-installed --force-reinstall --no-warn-conflicts -U -r res*/sta*/opt*.txt && clear && bash startup
